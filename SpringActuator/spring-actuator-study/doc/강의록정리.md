...
이전 예제 코드를 실행한후 웹브라우저에서 아래 링크 접근
...
- http://127.0.0.1:8080/actuator/metrics/http.server.requests 에 대해서도 위 방법으로 필터링
- http://127.0.0.1:8080/actuator/metrics/http.server.requests?tag=status:200

  - <span class=burk>"?tag=status:200"</span>

##### custom metric 을 만들기

tag 를 넣어서 필터링 되도록 custom metric 을 만들어봅시다.

app 내부적으로 queue 를 생성했다고 가정하고,

- 이 queue 에 push 한 횟수와
- pop 한 횟수를 구하는 counter 를 등록
...
- rest controller 에 아래처럼 위에서 만든 bean을 주입받고,
- push()와 pop()을 호출
...
- 테스트: http://127.0.0.1:8080/api/pop  / http://127.0.0.1:8080/api/push
- http://127.0.0.1:8080/actuator/metrics/myQueueCounter
  ```json
  {
    "name": "myQueueCounter",
    "measurements": [
  ...
  ```
- actuator 에서 값을 보면 tag 들이 나오며,
- tag를 통해 필터링된 값들을 확인할 수 있습니다.
  - http://127.0.0.1:8080/actuator/metrics/myQueueCounter?tag=type:push
...
###### @Counted 라는 어노테이션에도 tag 를 넣어봅시다.

- @Counted 는 아래처럼 기본제공되는 tag 들이 있습니다.
- http://127.0.0.1:8080/actuator/metrics/myCountedAnnotationCount
  - "http://127.0.0.1:8080/api/counted"  2,3번 호출하자
- 이것외에 추가로 넣어보겠습니다.

@Counted 어노테이션을 살펴보니 예상대로

- tag 관련 필드가 있으며, 설명을 보니 배열로 값을 넣되 key, value 순으로 넣어야 한다고 합니다.
- 따라서 무조건 짝수개로 값이 들어가야겠죠?
...
```
- rest api 를 1회 이상 호출 (http://127.0.0.1:8080/api/counted, http://127.0.0.1:8080/api/counted2)
- 브라우저에서 tag 가 잘 동작하는걸 알 수 있습니다.
  - http://127.0.0.1:8080/actuator/metrics/myCountedAnnotationCount?tag=type:test1

Counter 를 meterRegistry에 등록시

- tag 값이 다르면 다른 Counter 로 등록된다는것.
- Counter name 을 통해 그룹핑되고 tag 를 통해 필터링할 수 있다는 것을 꼭 기억
...
### 10. about metrics endpoint(Gauge)

[유튜브 강의 바로 가기](https://youtu.be/oGJlKymgixc)

Counter 타입

- 횟수값을 다루는데 주로 사용
- 양수값만 사용되고 값이 작아지지 않습니다.
- Counter api 단에서 increment 메서드만 제공되지, decrement 메서드가 없습

Gauge 는

- 자동차 속도 계기판과 같이 커졌다 작아졌다 할 수 있는 값에 사용
- 보통 cpu usage, mem usage , 사용중인 thread count, 사용중인 connection pool count 등에 사용
- thread count 라고 해서 Counter 를 쓰는게 맞지 않냐? 라고 생각할 수 있으나 사용중인 thread count는 커졌다 작아졌다 할 수 있으므로 Gauge 사용

Gauge 의 경우,

- 조회시도 할때만 해당 값을 계산
- cpu usage 를 예로 들자면, cpu usage 를 actuator 를 통해 노출할 필요가 있을때 cpu usage 관련 클래스를 이용해서 값을 조회한 후 Gauge metric 타입으로 노출하면 됩니다.
- 따라서 일반적으로 아래처럼 특정 metric 를 관리하는 객체를 파라미터로 전달하는 형식으로 등록하게 됩니다.
  ...
  Gauge gauge = Gauge
     ...
     .register(registry);
  ```
- Counter 에서 다룬 FunctionCounter 와 동일한 형식입니다.
...
테스트를 위해 임의로 특정 값을 리턴하는 service

- kafka 와 같은 queue 에 대기중인 수를 리턴하는 service 라고 가정하겠습니다.
...
- 생성한 gauge 변수는 사실 더 사용할 곳이 없으므로 굳이 변수를 만들 필요는 없습니다.
  ...
  - my.queue.size 로 이름을 등록했으므로 아래처럼 url을 입력하면 값이 잘 나오는걸 알 수 있습니다.
    - http://127.0.0.1:8080/actuator/metrics/my.queue.size
...
기존 코드를 보면 QueueManager bean 와 MeterRegistry bean이 필요해서 생성자 주입을 받았습니다.

- 이렇게 다른 bean이 필요한 경우가 대부분이므로, 좀 더 편리하게 사용하라고 **MeterBinder** 라는 인터페이스가 제공되며,
- 아래처럼 MeterBinder 를 implement 한건 bean으로 등록하면 필요한 bean들은 **자동으로 주입**이 됩니다.
...
### 11. about metrics endpoint ( Timer )

[유튜브1](https://youtu.be/aqkb1SHBNIc)
[유튜브2](https://youtu.be/LywjP3UDTcM)

#### timer 등록

builder 를 이용해서 timer 를 만든 후 meterRegistry 에 등록

- my.timer 라고 파라미터값을 넣었으므로 actuator 에서는 my.timer 가 path에 들어가게 됩니다.
- timer 의 경우 시간값을 측정하고 싶은 곳에 timer 로 감싸서 구현
- Timer 를 bean 으로 등록해둬야 합니다.
...
controller 내의 특정 메서드의 수행 시간을 timer metric 을 이용해서 측정

- Timer bean을 생성자 주입
- @Qualifier("myTimer")는 넣지 않아도 오류가 나지 않으나, timer 를 개인적으로 더 추가해서 테스트 하는 분들이 있을듯 하여 Timer 타입의 bean이 2개 이상 있어도 오류가 나지 않게 미리 @Qualifier 를 추가
...
```
- 3초 정도 걸리는 코드가 있다는 가정하여 sleep 3초를 넣어두었습니다.

#### timer test

method 가 실행되도록 http request 를 보냅니다.

- http://127.0.0.1:8080/api/timer <-- 거래
- http://127.0.0.1:8080/actuator/metrics/my.timer 으로 접속을 하면 횟수, 누적 수행시간, 최대 수행시간 항목이 나오는걸 알 수 있습니다.
- 3번 수행 했는데, 6번으로 나올까?
  ```json
  {
    "name": "my.timer",
...
- Runnable 실행 전후에 시간을 알아낸후 그걸 계산해서 수행시간을 측정
...
지금까지 알아본 방법은 Timer.record() 메서드의 파라미터에 Runnable을 넣어주는 방식이었습니다.

- 이 방식으로 대부분 해결되나, 좀 복잡한 로직에서 Timer 를 사용하고 싶다면 Timer.Sample 을 이용하는것도 좋은 방식입니다.
...
위 Timer.Sample 예제를 보니 timer 측정하고 싶은 코드 앞 뒤에 start와 stop 메서드를 호출하는 패턴입니다
...
```
- http request 를 날린 후 http://127.0.0.1:8080/actuator/metrics/my.timer3 에 접속하면 아래처럼 timer metric 정보를 볼 수 있습니다.
...
```json
{
  ...
  "availableTags": [
    ...
    },
    {
      "tag": "class",
      "values": [
        "me.developery.actuatorstudy.timer.TimerController"
      ]
    }
  ]
}
```
AOP를 이용하고 있으므로 일반적인 AOP의 제약사항이 그대로 적용됩니다.

- 즉 아래처럼 클래스 내부의 메서드들끼리 호출될 경우 AOP가 적용되지 않기에 결론적으로 my.timer4 는 동작하지 않습니다.
...
Counter metric 파트에서 자체 counter 관리 bean 이 있다면,

- 이를 재활용하기 위해 FunctionCounter 라는 클래스를 이용하면 된다고 배웠습니다.
- timer 도 동일하게 FunctionTimer 가 있으며 사용 패턴은 동일
- 아래처럼 자체적으로 시간을 관리하는 bean 이 있다고 가정
  - 테스트 용도이니, 현재 시간값을 count, 누적 시간 으로 지정
...
```
- timer 를 등록하기 위해
  - FunctionTimer 타입의 bean을 생성하며,
  - 이때 위에서 만든 bean을 주입받아서 사용하면 됩니다.
...
- 다소 복잡하나, 횟수와 누적시간을 가져오는 메서드를 지정해주고, 누적시간값의 단위 즉 초, 분, 시 등을 지정해주면 됩니다.
...
위에서는 FunctionTimer 자체를 bean 으로 등록했는데
...
```
[소스코드](https://github.com/ChunGeun-Yu/spring-actuator-study/tree/timerMetrics)

### 12. Metrics 정리

[동영상](https://youtu.be/h3LqD0edGE0)

이제까지 counter, gauge, timer 라는 metrics 에 대해 알아보았습니다. ( 그외에도 몇가지 metrics 가 있긴 합니다. https://micrometer.io/docs/concepts 참조 )

#### 1. annotation 지원

어노테이션이 지원되는 metric 은
...
void method2() {}
```
AOP를 통해 counter 와 timer 가 동작

- 둘 다 @Aspect와 @Around 관련 코드가 필요
- 그걸 CountedAspect 와 TimedAspect 클래스에 구현
-
...
- 그러나 gauge 의 경우 cpu usage, 배송중인 물건갯수, 현 시점의 재고수량 과 같이 비지니스와 연관된 계산식이 필요하므로 AOP로 만들기가 어렵습니다.
...
위 사용패턴1은 측정하고 싶은 코드에 metric 코드를 넣어주는 방식
- 그런데 app 자체적으로 횟수, 시간 등을 계산하는 클래스가 있다고 가정해봅시다. 
- 이럴 경우 app에서 자체적으로 제공하는 계산식에게 측정을 위임하고, 그 값을 metric에 등록만 하면 됩니다.

- 이를 위해 FunctionCounter 와 FunctionTimer 가 제공
...
위의 사용패턴2 방식으로 metric 등록해도 되나 MeterBinder 를 이용해서도 등록이 가능
...
```
## 섹션 4. spring boot admin 연동

### 13. spring boot admin 연동
[유튜브]https://youtu.be/YEfnxsrVPtw

이제까지는 actuator 의 값을 보기 위해 웹브라우저에 http://127.0.0.1:8080/actuator/ 로 시작하는 url 을 넣고 일일이 확인을 했습니다.

단순히 조회만 할것이면 웹브라우저로 가능하나, 
- 값을 변경하는 작업은 HTTP GET이 아닌 HTTP PUT, POST 와 같은 걸 사용해야 하는데 웹브라우저 자체는 이를 지원하지 않습니다. 게다가 json 형식으로 아주 많은 데이터가 나올 경우 눈으로 일일이 확인하기가 어렵습니다.
- 또한 여러 application이 있을때, ip가 서로 다를 것이므로 일일이 ip를 변경해가면서 값을 확인해야 합니다.

- 이런 불편함은 spring boot admin 이라는 걸 이용하면 많이 해소
...
우선 기존 spring boot 에 spring boot admin client 모듈을 설치 및 설정
...
이후 아래처럼 spring boot admin server 의 ip, port 정보를 기입
- 아직 우린 server 를 만들지 않았지만 localhost 에서 9090 포트로 만들것이므로 아래처럼 설정
...
- 마지막으로 application.properties 파일에 아래처럼 9090 포트를 적어줍니다. 
- - 이건 제 PC 에서 2대의 spring boot 를 동시에 구동해야 하다보니 port 충돌이 나지 않도록 하기 위해 임의로 설정한 값입니다. 
...
이제 client와 server 모두 구동
- 서버 주소인 http://127.0.0.1:9090/ 으로 접속해보면 아래처럼 다양한 정보를 볼 수 있습
- 단순 조회성은 굳이 spring boot admin을 사용할 필요가 없어 보이나 logger level 실시간 변경, thread dump 등의 HTTP PUT/POST 성의 actuator 작업은 spring boot admin  으로 쉽게 조작을 할 수 있으니 편리해보입니다.
...
- 그래서 실무에서는 spring boot admin 보다는 prometheus, grafana 와 같은 통합 모니터링 도구를 이용하는게 더 일반적
