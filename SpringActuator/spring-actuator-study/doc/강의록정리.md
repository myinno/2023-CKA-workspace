...
등록된 bean 리스트, cache 상태 등이 기본 제공
- 실무에서 사내 정책에 따라 사내 전용 endpoint 가 필요한 경우
...
- application에서 참조하는 라이브러리 이름과 버전 정보를 응답으로 내보내는 myLibraryInfo 라는 이름의 endpoint

### 기본 클래스 만들기
일반적으로 endpoint 는
- application 의 특정 정보를 응답으로 리턴
- 즉 cpu usage 를 응답으로 내보내고, 등록된 bean 정보를 응답
